...
from app.induction.heuristic import induce_heuristic  # noqa: E402
...
def main() -> None:
    ...
    print(f"seeded trace {trace.id} ({len(trace.events)} events) "
          f"and workflow {spec.id} ({len(spec.steps)} steps) into {DATA}")
